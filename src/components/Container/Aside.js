...
function Aside() {
  ...
  const { data, error } = useGetFavoriteMoviesQuery(sessionId, { skip: !sessionId });
  return (
    <aside className=" w-1/5 py-10 px-10  min-w-min  border-l border-gray-300 dark:border-zinc-700 hidden lg:block">
      <div className="mt-10">
        <span className="font-semibold text-gray-700 dark:text-white">Your Favorites</span>

        {!sessionId ? (
          <div>
            <p className="dark:text-white rounded-md border border-yellow-600 p-3 mt-5 text-center mb-10">
              You don't have an active session. To display your favorites list, you need to log in.
            </p>
          ...
          <ul className="mt-4 text-gray-400 text-xs space-y-3">
            {data?.results?.slice(0, 3).map((el) => (
              <Item title={el.title} img={el.poster_path} score={el.vote_average} date={el.release_date} />
            ))}

            {data?.results?.length > 3 && (
              <li className="pt-1">
                <Link to="/favorite-movies">
                  <button className="px-5 py-2.5 bg-red-600 hover:bg-red-700 rounded-lg text-center font-medium block text-white w-full">
                    See More
                  </button>
                </Link>
              </li>
...
function Item({ title, img, score, date }) {
  return (
    <li className="flex space-x-3 ">
      <img src={`https://image.tmdb.org/t/p/w300${img}`} className="object-cover w-1/3 rounded-md" alt="" />
      <div className="flex flex-col justify-between  ">
        <div className="flex flex-col space-y-1">
          <span className="text-gray-800 dark:text-white font-semibold">{title}</span>
          <span className="text-xs hidden xl:block dark:text-gray-400 text-gray-700">Release: {date}</span>
        </div>
